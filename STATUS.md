...
## 2026-07-09 Task 4 report

- Fixed the Signals Run now path so a `201` response with `run.status=failed` now preserves the returned run, marks the workspace status as failed, and shows a Ukrainian failure toast instead of a success/scheduled message.
- Added a polished Ukrainian empty state for an empty authoritative Signals bank, with Run now or enable actions and a separate filtered-empty path.
- Added a regression script at `scripts/test-signals-ui-state.mjs`.
- Verified with `node scripts/test-signals-ui-state.mjs` and `npm.cmd run build`.
